      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 bg-white border border-obs-border rounded-lg overflow-hidden mb-16">
          <div className="p-8 border-b md:border-b-0 md:border-r border-obs-border hover:bg-surface transition-colors">
            <p className="text-[10px] font-mono font-bold text-text-muted uppercase tracking-[0.2em] mb-4">Total Scans</p>
          ...
          <div className="p-8 border-b md:border-b-0 md:border-r border-obs-border hover:bg-surface transition-colors">
            <p className="text-[10px] font-mono font-bold text-text-muted uppercase tracking-[0.2em] mb-4">In Progress</p>
          ...
          <div className="p-8 hover:bg-surface transition-colors">
            <p className="text-[10px] font-mono font-bold text-text-muted uppercase tracking-[0.2em] mb-4">Workspace Uptime</p>
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <button
              onClick={() => setShowNewCheck(true)}
              className="group relative overflow-hidden p-6 bg-white border border-obs-border rounded-lg hover:border-amber hover:shadow-[0_8px_24px_-12px_rgba(28,12,158,0.35)] transition-all text-left"
            >
              <div className="relative w-10 h-10 rounded-lg bg-amber flex items-center justify-center mb-5 ring-1 ring-amber-hover/60">
                <LuCompass className="text-white text-xl" />
                <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-cyan ring-2 ring-white" />
              </div>
              <h4 className="text-sm font-semibold text-text mb-1.5">Landscape Scan</h4>
            ...
            <button
              onClick={() => setShowNewCheck(true)}
              className="group relative overflow-hidden p-6 bg-white border border-obs-border rounded-lg hover:border-cyan hover:shadow-[0_8px_24px_-12px_rgba(15,151,170,0.35)] transition-all text-left"
            >
              <div className="relative w-10 h-10 rounded-lg bg-cyan flex items-center justify-center mb-5 ring-1 ring-cyan-hover/60">
                <LuFileSearch className="text-white text-xl" />
                <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-amber ring-2 ring-white" />
              </div>
              <h4 className="text-sm font-semibold text-text mb-1.5">Prior-Art Search</h4>
              <p className="text-xs text-text-secondary leading-relaxed mb-4">
                Seed with an invention disclosure or claim draft. Get ranked prior-art hits across patents and literature with the specific passages that conflict.
              </p>
          ...
          <div>
            ...
            {loading ? (
              <div className="p-12 border border-obs-border border-dashed rounded-lg text-center">
                 <LuLoader className="animate-spin text-amber text-3xl mb-4 mx-auto" />
              ...
              <div className="py-20 flex flex-col items-center justify-center text-center border border-obs-border rounded-lg bg-surface">
                <div className="relative w-16 h-16 rounded-xl bg-amber flex items-center justify-center mb-6 ring-1 ring-amber-hover/60">
                  <LuFileSearch className="text-white text-3xl" />
                  <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-cyan ring-2 ring-surface" />
                </div>
                <h3 className="text-xl font-display font-semibold mb-2">No scans yet</h3>
...
function SessionCard({ session, onClick }: { session: Session; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-sm font-semibold text-text mb-6 line-clamp-2 leading-relaxed min-h-12">
        {session.claim}
      </h3>
