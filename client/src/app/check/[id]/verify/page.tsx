...
const pipelineStages: { id: string; label: string; icon: IconType; description: string }[] = [
    { id: "finding", label: "Evidence", icon: MdLightbulb, description: "Original evidence extracted from source" },
    { id: "questions", label: "Verification Questions", icon: MdQuiz, description: "Generate targeted questions to verify the claim" },
    { id: "evidence", label: "Evidence Search", icon: MdSearch, description: "Search for corroborating or contradicting evidence" },
    { id: "scoring", label: "Confidence Score", icon: MdSpeed, description: "Calculate final confidence based on evidence" },
];
            ...
            {loading ? (
                ...
                <div className="flex-1 flex items-center justify-center">
                    <div className="text-center">
                        <MdVerified className="text-4xl text-text-muted mb-3 block mx-auto" />
                ...
                <div className="flex-1 flex flex-col md:flex-row max-w-7xl mx-auto w-full overflow-hidden">
                    ...
                    <main className="flex-1 p-4 md:p-8 space-y-8 overflow-y-auto max-h-[calc(100vh-180px)]">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <MiniStat icon={MdVerified} label="Verified" value={displayStats.verified} color="text-emerald" />
                            <MiniStat icon={MdFlag} label="Flagged" value={displayStats.flagged} color="text-gold" />
                            <MiniStat icon={MdDangerous} label="Rejected" value={displayStats.rejected} color="text-rose" />
                            <MiniStat icon={MdHourglassEmpty} label="Pending" value={displayStats.pending} color="text-text-muted" />
                        </div>
                            ...
                            <>
                                                ...
                                                <div key={stage.id} className="flex items-start flex-1">
                                                    <div className={`flex-1 relative rounded-2xl p-5 border transition-all ${isActive
                                                        ...
                                                        <div className="flex items-center gap-2 mb-3">
                                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isCompleted ? "bg-emerald/10" : isActive ? "bg-amber/20" : "bg-obs-border"}`}>
                                                                {isCompleted ? (
                                                                    <MdCheckCircle className={`text-base ${isActive ? "text-amber" : "text-emerald"}`} />
                                                                ) : (
                                                                    <stage.icon className={`text-base ${isActive ? "text-amber" : "text-text-muted"}`} />
                                                                )}
                                                            </div>
                                                            <div>
                                                    ...
                                                    {i < pipelineStages.length - 1 && (
                                                        <div className="flex items-center pt-8 mx-2">
                                                            ...
                                                            <MdChevronRight className={`text-sm ${isCompleted ? "text-emerald" : "text-text-muted"}`} />
                                                        </div>
                                                    )}
                                                </div>
                                            );
                                ...
                                {selectedFinding.questions_asked && selectedFinding.questions_asked.length > 0 && (
                                    <div className="obs-card">
                                        <div className="flex items-center gap-2 mb-4">
                                            <MdQuiz className="text-amber text-xl" />
                                                            ...
                                                            <div className="flex items-center gap-3 text-xs">
                                                                {q.supports_original !== null && (
                                                                    <span className={`flex items-center gap-1 ${q.supports_original ? "text-emerald" : "text-rose"}`}>
                                                                        {q.supports_original ? <MdCheckCircle className="text-sm" /> : <MdCancel className="text-sm" />}
                                ...
                                {selectedFinding.corrections_made && selectedFinding.corrections_made.length > 0 && (
                                    <div className="obs-card border-gold/30 bg-gold/5">
                                        <div className="flex items-center gap-2 mb-3">
                                            <MdEditNote className="text-gold text-xl" />
                                        ...
                                        <ul className="space-y-2">
                                            {selectedFinding.corrections_made.map((correction, idx) => (
                                                <li key={idx} className="flex items-start gap-2 text-xs text-text-secondary">
                                                    <MdArrowRight className="text-sm text-gold mt-0.5 shrink-0" />
...
function MiniStat({ icon: Icon, label, value, color }: { icon: IconType; label: string; value: number; color: string }) {
    return (
