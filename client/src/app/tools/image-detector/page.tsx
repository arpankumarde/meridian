...
import { LuArrowLeft, LuScanSearch, LuUpload, LuLink, LuLoader, LuImage, LuCloudUpload, LuX, LuCircleAlert, LuCamera, LuCircleHelp, LuSparkles, LuEye } from "react-icons/lu";
...
export default function ImageDetectorPage() {
  ...
  const verdictConfig: Record<string, { label: string; color: string; bg: string; icon: IconType }> = {
    ai_generated: { label: "AI Generated", color: "text-rose", bg: "bg-rose-soft", icon: LuSparkles },
    human_created: { label: "Human Created", color: "text-emerald", bg: "bg-emerald-soft", icon: LuCamera },
    inconclusive: { label: "Inconclusive", color: "text-text-muted", bg: "bg-surface", icon: LuCircleHelp },
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-rose-soft border border-rose/10 flex items-center justify-center text-rose">
                <LuScanSearch className="text-lg" />
              </div>
              <div>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 py-12">
        ...
        <div className="mb-12">
          ...
          <p className="text-text-secondary text-sm mb-6">
            Upload an image or paste a URL. Meridian performs forensic metadata analysis - EXIF data, dimensions, color properties, embedded generation parameters - to determine if an image was AI-generated.
          </p>
          ...
          <div className="flex gap-1 p-1 bg-surface rounded-lg w-fit mb-6">
            ...
            >
              <LuUpload className="text-sm align-middle mr-1 inline-block" />
              Upload
            </button>
            <button
              onClick={() => setMode("url")}
            ...
            >
              <LuLink className="text-sm align-middle mr-1 inline-block" />
              URL
            </button>
          </div>
          ...
          {mode === "url" && (
            <form onSubmit={handleSubmitUrl} className="flex gap-3">
              <div className="flex-1 relative">
                <LuImage className="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-muted text-lg" />
                <input
                  type="url"
                  value={url}
              ...
              >
                {loading ? (
                  ...
                  <LuScanSearch className="text-sm" />
                )}
          ...
          {mode === "upload" && (
            <div className="space-y-4">
              {!selectedFile ? (
                ...
                >
                  <LuCloudUpload className={`text-4xl mb-3 block ${dragOver ? "text-amber" : "text-text-muted"}`} />
          ...
          {error && (
            <div className="mt-4 p-4 bg-rose-soft border border-rose/20 rounded-lg flex items-start gap-3">
              <LuCircleAlert className="text-rose text-lg shrink-0" />
        ...
        {loading && (
          <div className="border border-obs-border border-dashed rounded-lg p-16 text-center">
            <LuLoader className="animate-spin text-amber text-4xl mb-4 block mx-auto" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {result.indicators.map((ind, i) => (
                      <div key={i} className="flex items-start gap-2.5 p-3 bg-surface rounded-lg">
                        <LuEye className={`text-sm mt-0.5 ${vc.color}`} />
        ...
        {!result && !loading && !error && !selectedFile && (
          <div className="border border-obs-border rounded-lg p-16 text-center bg-surface/30">
            <div className="w-16 h-16 rounded-xl bg-rose-soft flex items-center justify-center mx-auto mb-6">
              <LuScanSearch className="text-rose text-3xl" />
            </div>
