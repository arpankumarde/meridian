...
export default function PlagiarismCheckerPage() {
  ...
  const verdictConfig: Record<string, { label: string; color: string; bg: string; icon: IconType }> = {
    original: { label: "Original Content", color: "text-emerald", bg: "bg-emerald-soft", icon: LuBadgeCheck },
    partially_plagiarized: { label: "Partially Plagiarized", color: "text-gold", bg: "bg-gold-soft", icon: LuCopy },
    heavily_plagiarized: { label: "Heavily Plagiarized", color: "text-rose", bg: "bg-rose-soft", icon: LuOctagonAlert },
  };
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 py-12">
        ...
        <div className="mb-12">
          ...
          <p className="text-text-secondary text-sm mb-6">
            Paste a URL or text to check. Meridian will extract key passages and search the web for matching content.
          </p>
            ...
            <div className="space-y-6">
                    ...
                    {result.matches.map((match, i) => (
                      <div key={i} className="border border-obs-border rounded-lg p-4 hover:border-amber/30 transition-colors">
                        <div className="flex items-start justify-between gap-4 mb-2">
                          ...
                          >
                            <LuExternalLink className="text-sm text-text-muted shrink-0" />
              ...
              {result.matches.length === 0 && (
                <div className="border border-obs-border rounded-lg p-8 bg-white text-center">
                  <LuCircleCheck className="text-emerald text-3xl mb-3 block mx-auto" />
        ...
        {!result && !loading && !error && (
          <div className="border border-obs-border rounded-lg p-16 text-center bg-surface/30">
            <div className="w-16 h-16 rounded-xl bg-cyan-soft flex items-center justify-center mx-auto mb-6">
              <LuCopy className="text-cyan text-3xl" />
            </div>
