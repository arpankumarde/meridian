...
import { LuArrowLeft, LuBot, LuLink, LuNotebookPen, LuLoader, LuSearch, LuCircleAlert, LuArrowLeftRight, LuUser, LuCircleHelp, LuArrowRight, LuExternalLink } from "react-icons/lu";
...
export default function AIDetectorPage() {
  ...
  const verdictConfig: Record<string, { label: string; color: string; bg: string; icon: IconType }> = {
    ai_generated: { label: "AI Generated", color: "text-rose", bg: "bg-rose-soft", icon: LuBot },
    human_written: { label: "Human Written", color: "text-emerald", bg: "bg-emerald-soft", icon: LuUser },
    mixed: { label: "Mixed Content", color: "text-gold", bg: "bg-gold-soft", icon: LuArrowLeftRight },
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 py-12">
        ...
        <div className="mb-12">
          ...
          <p className="text-text-secondary text-sm mb-6">
            Paste a URL or text to analyze. Meridian will check for AI-generation patterns in writing style and structure.
          </p>
            ...
            {mode === "url" ? (
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  <LuLink className="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-muted text-lg" />
                  <input
                    type="url"
                    value={url}
        ...
        {loading && (
          <div className="border border-obs-border border-dashed rounded-lg p-16 text-center">
            <LuLoader className="animate-spin text-amber text-4xl mb-4 block" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {result.indicators.map((ind, i) => (
                      <div key={i} className="flex items-start gap-2.5 p-3 bg-surface rounded-lg">
                        <LuArrowRight className={`text-sm mt-0.5 ${vc.color}`} />
        ...
        {!result && !loading && !error && (
          <div className="border border-obs-border rounded-lg p-16 text-center bg-surface/30">
            <div className="w-16 h-16 rounded-xl bg-violet-soft flex items-center justify-center mx-auto mb-6">
              <LuBot className="text-violet text-3xl" />
            </div>
