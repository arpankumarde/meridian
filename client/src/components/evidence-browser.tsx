    ...
    (async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:9090/api/sessions/${sessionId}/evidence?${params.toString()}`,
        );
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {loadingEvidence ? (
              ...
              <div className="text-sm text-text-secondary p-4 text-center">
                <MdScience className="text-3xl text-text-muted block mb-2 mx-auto" />
              ...
              <>
                {evidence.slice(0, visibleCount).map((item) => {
                  const isActive =
                    item.id === (selectedEvidence?.id ?? evidence[0]?.id);
                  return (
                    <button
                      key={item.id}
                      onClick={() => setSelectedId(item.id)}
                      className={`w-full text-left flex flex-col gap-2 p-3 rounded-2xl transition-all border cursor-pointer ${
                        isActive
                          ? "bg-surface-hover border-amber/40 ring-1 ring-amber/20"
                          : "bg-surface-inset/60 border-obs-border hover:border-amber/30"
                      }`}
                      style={
                        isActive ? { boxShadow: "var(--shadow-md)" } : undefined
                      }
                    >
                      <div className="flex items-center gap-2 text-xs">
                        <span
                          className={`obs-badge ${getEvidenceBadge(item.finding_type)}`}
                        >
                          {item.finding_type}
                        </span>
          ...
          {selectedEvidence ? (
            <>
              <div className="px-5 py-3 border-b border-obs-border bg-surface">
                <div className="flex items-center gap-2 mb-1.5">
                  <span
                    className={`obs-badge ${getEvidenceBadge(selectedEvidence.finding_type)}`}
                  >
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                {selectedEvidence.source_url && (
                  <div>
                    ...
                    >
                      <MdOpenInNew className="text-sm mt-0.5 text-text-muted" />
            ...
            <div className="flex-1 flex items-center justify-center text-text-muted">
              <div className="text-center">
                <MdDescription className="text-4xl text-text-muted mb-2 block mx-auto" />
                <p className="text-sm">
...
function ConfidenceRing({ value }: { value: number }) {
  ...
  const color =
    pct >= 70 ? "text-emerald" : pct >= 40 ? "text-gold" : "text-rose";
  return (
...
function ExpandableText({
  ...
  return (
    <div>
      <p
        className={`text-sm text-text leading-snug ${!expanded && isLong ? `line-clamp-${maxLines}` : ""}`}
      >
        {text}
      </p>
      {isLong && (
