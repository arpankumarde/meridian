...
@router.post("/detect-ai-text", response_model=DetectionResult)
async def detect_ai_text(req: AnalyzeTextRequest):
    """Detect whether pasted text is AI-generated."""
    text_input = req.text.strip()
    if not text_input:
        raise HTTPException(status_code=422, detail="No text provided")

    content = text_input[:12000]
    preview = text_input[:300]

    prompt = f"""Analyze the following text and determine whether it was written by a human or generated by AI (like ChatGPT, Claude, etc.).
...
Focus on: repetitive sentence structures, lack of personal voice, hedging language ("It's important to note"), formulaic transitions, unusual uniformity in paragraph length, lack of specific personal experience, generic examples, and characteristic AI phrasings."""
    ...
    try:
        ...
        logger.error("AI text detection failed: %s", e)
        raise HTTPException(status_code=500, detail=f"Analysis failed: {str(e)}")
...
@router.post("/check-plagiarism-text", response_model=PlagiarismResult)
async def check_plagiarism_text(req: PlagiarismTextRequest):
    """Check pasted text for potential plagiarism."""
    text_input = req.text.strip()
    if not text_input:
        raise HTTPException(status_code=422, detail="No text provided")
...
Return ONLY the passages, one per line, prefixed with PASSAGE: (no numbering, no quotes)."""
    ...
    for passage in passages:
        try:
            ...
            results = await bd.search(search_query, num=5, engine="google")
            for r in results:
                if r.url not in [m.source_url for m in matches]:
                    matches.append(PlagiarismMatch(
                        passage=passage[:200],
                        source_url=r.url,
                        source_title=r.title,
