...
os.environ.setdefault("MERIDIAN_IN_API", "1")
...
_API_KEY = os.environ.get("MERIDIAN_API_KEY")
...
#
# API key auth (optional — only active when MERIDIAN_API_KEY is set)
...
async def test_emit_event(session_id: str, event_type: str = "test", message: str = "Test event"):
    ...
    if not os.environ.get("MERIDIAN_DEBUG"):
...
if __name__ == "__main__":
    ...
    # reload=True forces WindowsSelectorEventLoopPolicy on Windows, which breaks
    # subprocess spawning (claude-agent-sdk). Disable reload on Windows.
    uvicorn.run(
        "api.server:app",
        host="0.0.0.0",
        port=9090,
